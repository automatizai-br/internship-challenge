...
def _remover_duplicados(registros: Iterable[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    O PNCP às vezes devolve a mesma contratação em páginas diferentes quando o
    conjunto de resultados muda entre as requisições, então vale filtrar.
    """
    vistos: set[str] = set()
    unicos: list[dict[str, Any]] = []

    for registro in registros:
        chave = (registro.get("orgaoEntidade") or {}).get("cnpj")
